...
int main ()
{
    ...
    int ex[9], ey[9], kb[9];
    float f[21][21][21], ft[21][21][21];
    float wt[5];
    int isn[nx][ny];
    float st[9], ux, uy, rho, term1, term2, u2;
    float source[9];
    int time=20000;
    float dpdx = 1.0e-5;
    float tau = 0.8;
    ...
    */
    ...
    for (i=0;i<nx;i++)
    {
        for(j=1;j<ny-1;j++)
        {
            ...
            for (a=0;a<9;a++)
            {
                f[a][i][j] = wt[a]*rho0; //corresponds to zero velicty
            }
         ...
         for (i=0;i<nx;i++)
         {
             for(j=1;j<ny-1;j++)
             {
                 ...
                 for (a=0;a<9;a++)
                 {
                     ...
                     term2 = term1*term1;
                     source[a] = (1.0-0.05/tau)*wt[a]*(3.0*(ex[a]-ux) + 9.0*(ex[a]*ux + ey[a]*uy)*ex[a])*dpdx;
